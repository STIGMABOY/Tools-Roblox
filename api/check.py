...
# Import auth functions for user verification
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            # Check user authentication and expiration for cookie checker actions
            if action in ['start', 'stop', 'test', 'clear', 'export']:
                # Get token from Authorization header or data
                token = self.headers.get('Authorization', '').replace('Bearer ', '') or data.get('token', '')
                ...
                # Verify user token
                user_result = verify_user_token(token)
                ...
                if user_result['data'].get('is_expired', False):
                    ...
                    self.wfile.write(json.dumps({
                        'success': False,
                        'error': 'Account expired. Please contact admin to reactivate your account.'
                    }).encode())
                    return
